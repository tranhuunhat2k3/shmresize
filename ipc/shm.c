...
#include <linux/fs.h>   //Add library 
#include <linux/fcntl.h> //for ftruncate denifitions
#include <linux/uaccess.h>
#include <linux/pagemap.h> // Cho add_to_page_cache_lru
...
long ksys_shmresize(int shmid, size_t new_size) {
	...
	//Create a new file for the resize segment
	new_file = shmem_kernel_file_setup("SYS_SHMRESIZE", new_size, 0);
	if (IS_ERR(new_file)) {
		err = PTR_ERR(new_file);
		goto unlock;
	}

	//Alocate a temporary buffer to hold old data for copying
...
//System call definition for user-space address
